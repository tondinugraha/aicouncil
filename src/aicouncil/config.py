...
# Backward compatibility — ModelConfig and get_model_config
# These delegate to the main Config now.
ModelCategory = str  # Was Literal["fast", "default", "reasoning"]


class ModelConfig(BaseModel):
    """Configuration for model selection by task category.

    Backward-compatible wrapper. New code should use Config.resolve_model().
    """
...
def get_model_config() -> ModelConfig:
    ...
    Now reads from YAML config via Config object.
    Falls back to OPENROUTER_MODELS env var, then OPENROUTER_MODEL env var.
    """
    import json
    ...
    # First try: env var override (escape hatch)
    ...
    # Second: derive from Config's tool_overrides
    try:
        config = get_config()
        model = config.default_model
        _model_config = ModelConfig(
            fast=config.resolve_model("brainstorm"),
            default=model,
            reasoning=config.resolve_model("critique"),
        )
        return _model_config
    except ConfigError:
        pass
